...
use surveyor_physics::{
    simulation::SimClock,
    spacecraft::{OrbitalDynamics, SpacecraftModel},
    SimulationTime,
};
...
pub fn compute_lander_state_from_simulation(
    ...
    let lander_quat_prev = Quat::from_xyzw(
        q_prev[1] as f32,
        q_prev[2] as f32,
        q_prev[3] as f32,
        q_prev[0] as f32,
    );
    let lander_quat_next = Quat::from_xyzw(
        q_next[1] as f32,
        q_next[2] as f32,
        q_next[3] as f32,
        q_next[0] as f32,
...
pub fn render_lander_state(
    ...
    if let Some(lander_state) = lander_state.read().last() {
        ...
        let (new_grid_cell, new_translation) = settings.translation_to_grid(lander_state.pos);
